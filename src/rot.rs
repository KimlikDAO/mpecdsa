...
impl ROTSender {
    ...
    pub fn decode<T: Read>(
        ...
        ro: &DyadicROTagger,
        recv: &mut T,
    ) -> io::Result<([u8; HASH_SIZE], [u8; HASH_SIZE])> {
    ...
    pub fn new<T: Read>(ro: &DyadicROTagger, recv: &mut T) -> Result<ROTRecver, MPECDSAError> {
        ...
        if prover_honest {
            ...
            Err(MPECDSAError::Proof(ProofError::new(
                "Proof of Knowledge failed for ROT secret key (sender cheated)",
            )))
        }
    }

    pub fn choose<T: Write>(
        &mut self,
        choice_bit: bool,
        ro: &DyadicROTagger,
        rng: &mut dyn Rng,
...
impl ROTSendVerifier {
    pub fn new<T: Write>(
        ...
        ro: &DyadicROTagger,
        send: &mut T,
    ) -> Result<ROTSendVerifier, MPECDSAError> {
    ...
    pub fn open<T1: Read, T2: Write>(
        ...
        if vec_eq(&chal_msg[..], &self.exp_chal[..]) {
            ...
            Err(MPECDSAError::Proof(ProofError::new(
                "Verification Failed for ROT (receiver cheated)",
            )))
        }
    }
}
...
impl ROTRecvVerifier {
    pub fn new<T1: Read, T2: Write>(
        ...
        recv: &mut T1,
        send: &mut T2,
    ) -> Result<ROTRecvVerifier, MPECDSAError> {
    ...
    pub fn open<T: Read>(&mut self, recv: &mut T) -> Result<(), MPECDSAError> {
        ...
        if vec_eq(&exp_com_msg, &self.com_msg) && vec_eq(&chosen_msg_com, &self.hashed_chosen_msg) {
            ...
            Err(MPECDSAError::Proof(ProofError::new(
                "Verification Failed for ROT (sender cheated)",
            )))
        }
    }
}
...
// for convenience:
pub fn rot_send_batch<T1: Read, T2: Write>(
    count: usize,
    ro: &DyadicROTagger,
    rng: &mut dyn Rng,
    recv: &mut T1,
    send: &mut T2,
) -> Result<Vec<([u8; HASH_SIZE], [u8; HASH_SIZE])>, MPECDSAError> {
...
pub fn rot_recv_batch<T1: Read, T2: Write>(
    choice_bits: &[bool],
    ro: &DyadicROTagger,
    rng: &mut dyn Rng,
    recv: &mut T1,
    send: &mut T2,
) -> Result<Vec<[u8; HASH_SIZE]>, MPECDSAError> {
...
mod tests {
    ...
    fn test_rot() {
        ...
        let child = thread::spawn(move || {
            ...
            rot_send_batch(
                N,
                &mut ro.get_dyadic_tagger(1).unwrap(),
                &mut rng,
                r1[1].as_mut().unwrap(),
                s1[1].as_mut().unwrap(),
            )
        ...
        rot_recv_batch(
            &choice_bits,
            &mut ro.get_dyadic_tagger(0).unwrap(),
            &mut rng,
            r2[0].as_mut().unwrap(),
            s2[0].as_mut().unwrap(),
        )
