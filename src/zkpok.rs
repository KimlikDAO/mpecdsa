...
pub fn prove_dl_fs<T:Write>(x:&SecpOrd, gx:&Secp, ro: &dyn ro::ModelessROTagger, rng:&mut dyn Rng, send:&mut T) -> Result<(),MPECDSAError> {
	...
	randcommitment.to_bytes(&mut buf[(Secp::NBYTES+RO_TAG_SIZE)..(2*Secp::NBYTES+RO_TAG_SIZE)]);
...
pub fn verify_dl_fs<T:Read>(gx:&Secp, ro: &ModelessDyadicROTagger, recv:&mut T) -> Result<bool,MPECDSAError> {
	let mut buf = [0u8;2*Secp::NBYTES + SecpOrd::NBYTES + RO_TAG_SIZE];
	gx.to_bytes(&mut buf[RO_TAG_SIZE..(Secp::NBYTES + RO_TAG_SIZE)]);
	...
	let randcommitment: Secp = Secp::from_bytes(&buf[(Secp::NBYTES + RO_TAG_SIZE)..(2*Secp::NBYTES + RO_TAG_SIZE)]);

	let mut challenge = [0u8; HASH_SIZE];
	hash(&mut challenge, &buf[0..(2*Secp::NBYTES + RO_TAG_SIZE)]); // TODO: tag

	let challenge = SecpOrd::from_bytes(&challenge[..]);
	let z = SecpOrd::from_bytes(&buf[(2*Secp::NBYTES+RO_TAG_SIZE)..(2*Secp::NBYTES+SecpOrd::NBYTES+RO_TAG_SIZE)]);
...
pub fn prove_dl_fs_to_com(x:&SecpOrd, gx:&Secp, ro: &dyn ModelessROTagger, rng:&mut dyn Rng) -> Result<([u8;HASH_SIZE], [u8;SecpOrd::NBYTES + Secp::NBYTES]),MPECDSAError> {
...
pub fn verify_dl_fs_with_com<T:Read>(gx:&Secp, proofcommitment:&[u8;HASH_SIZE], ro: &ModelessDyadicROTagger, recv:&mut T) -> Result<bool,MPECDSAError> {
	let mut buf = [0u8; 2*Secp::NBYTES+SecpOrd::NBYTES+RO_TAG_SIZE];
	...
	buf[(2*Secp::NBYTES+SecpOrd::NBYTES)..].copy_from_slice(&ro.next_dyadic_counterparty_tag()?);

	let mut exp_commitment = [0u8; HASH_SIZE];
	hash(&mut exp_commitment, &buf[Secp::NBYTES..(2*Secp::NBYTES+SecpOrd::NBYTES+RO_TAG_SIZE)]);

	Ok(pass && (proofcommitment==&exp_commitment))
}

pub fn verify_dl_fs_with_com_grouped<T:Read>(gx:&[Secp], proofcommitment:&[[u8;HASH_SIZE]], counterparties: &[usize], ro: &dyn ModelessROTagger, recv:&mut[&mut Option<T>]) -> Result<bool,MPECDSAError> {
	...
	for ii in 0..recv.len() {
		...
		if recv[ii].is_some() {
			...
			recv[ii].as_mut().unwrap().read_exact(&mut buf[(RO_TAG_SIZE + Secp::NBYTES)..(2*Secp::NBYTES+SecpOrd::NBYTES + RO_TAG_SIZE)])?;
			
			randcommitment = Secp::op(&randcommitment, &Secp::from_bytes(&buf[(Secp::NBYTES+RO_TAG_SIZE)..(2*Secp::NBYTES+RO_TAG_SIZE)]));
			...
			z = z.add(&SecpOrd::from_bytes(&buf[(2*Secp::NBYTES+RO_TAG_SIZE)..(2*Secp::NBYTES+SecpOrd::NBYTES+RO_TAG_SIZE)]));

			buf[(2*Secp::NBYTES+SecpOrd::NBYTES + RO_TAG_SIZE)..].copy_from_slice(&ro.next_counterparty_tag(counterparties[ii])?);
			let mut exp_commitment = [0u8; HASH_SIZE];
			hash(&mut exp_commitment, &buf[(RO_TAG_SIZE+Secp::NBYTES)..(2*Secp::NBYTES+SecpOrd::NBYTES+2*RO_TAG_SIZE)]); // TODO: tag
