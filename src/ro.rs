...
pub trait ROTagger {
    fn advance_counterparty_broadcast_counter(&self, _: usize, _: u64) -> Result<(), MPECDSAError>;
    fn next_counterparty_broadcast_tag(&self, _: usize) -> Result<[u8; RO_TAG_SIZE], MPECDSAError>;
    fn allocate_counterparty_broadcast_range(
        &self,
        counterparty: usize,
        length: u64,
    ) -> Result<TagRange, MPECDSAError>;

    fn advance_counterparty_dyadic_counter(&self, _: usize, _: u64) -> Result<(), MPECDSAError>;
    fn next_counterparty_dyadic_tag(
        &self,
        counterparty: usize,
    ) -> Result<[u8; RO_TAG_SIZE], MPECDSAError>;
    fn allocate_counterparty_dyadic_range(
        &self,
        counterparty: usize,
        length: u64,
    ) -> Result<TagRange, MPECDSAError>;
...
pub trait ModelessROTagger {
    ...
    fn next_counterparty_tag(&self, _: usize) -> Result<[u8; RO_TAG_SIZE], MPECDSAError>;
    fn allocate_counterparty_range(
        &self,
        counterparty: usize,
        length: u64,
    ) -> Result<TagRange, MPECDSAError>;
...
fn party_broadcast_base(
    ...
    for ii in 0..puids.len() {
        if subgroup_mask[ii] {
            hashin[(ii * HASH_SIZE + 8)..((ii + 1) * HASH_SIZE + 8)]
                .copy_from_slice(&puids[ii][..]);
        }
    }
...
impl GroupROTagger {
    ...
    pub fn from_network_unverified<TR: Read, TW: Write>(
        playerindex: usize,
        rng: &mut dyn Rng,
        recv: &mut [Option<&mut TR>],
        send: &mut [Option<&mut TW>],
    ) -> Result<GroupROTagger, MPECDSAError> {
        if recv.len() != send.len() {
            panic!("Number of Send streams does not match number of Recv streams");
        }
        ...
        for ii in 0..playercount {
            if ii != playerindex {
                send[ii].as_mut().unwrap().write(
                    &puid_seed[(playerindex * HASH_SIZE)..((playerindex + 1) * HASH_SIZE)],
                )?;
    ...
    fn from_seed(
        ...
        if subgroup_mask.len() != playercount {
            panic!("Subgroup mask length does not match player count");
        }
        if !subgroup_mask[playerindex] {
    ...
    pub fn apply_subgroup_list(&mut self, list: &[usize]) -> Result<(), MPECDSAError> {
        if list.len() > self.puids.len() {
            panic!("Subgroup list length greater than player count");
        }
        let mut mask = vec![false; self.puids.len()];
        for user in list {
            if *user >= mask.len() {
                return Err(MPECDSAError::General(GeneralError::new(
                    "Subgroup list contains invalid user",
                )));
            };
            mask[*user] = true;
        }
    ...
    pub fn fork_counterparty_tagger(
        &self,
        counterparty: usize,
    ) -> Result<GroupROTagger, MPECDSAError> {
    ...
    pub fn get_dyadic_tagger<'a>(
        &'a self,
        counterparty: usize,
    ) -> Result<DyadicROTagger<'a>, MPECDSAError> {
        let supercounterparty =
            self.subgroup_map_to_super[counterparty].ok_or(MPECDSAError::General(
                GeneralError::new(&format!("Invalid counterparty {}.", counterparty)),
            ))?;
...
impl ROTagger for GroupROTagger {
    fn advance_counterparty_broadcast_counter(
        &self,
        counterparty: usize,
        tagindex: u64,
    ) -> Result<(), MPECDSAError> {
        ...
        if oldcounter > tagindex {
            Err(MPECDSAError::Proof(ProofError::new(&format!(
                "Party {}/{} (subgroup/supergroup) attempted to reuse Random Oracle tag",
                counterparty, supercounterparty
            ))))
        } else {
            Ok(())
        }
    ...
    fn next_counterparty_broadcast_tag(
        &self,
        counterparty: usize,
    ) -> Result<[u8; RO_TAG_SIZE], MPECDSAError> {
        let supercounterparty =
    ...
    fn advance_counterparty_dyadic_counter(
        &self,
        counterparty: usize,
        tagindex: u64,
    ) -> Result<(), MPECDSAError> {
...
impl<'a> DyadicROTagger<'a> {
    ...
    pub fn next_dyadic_counterparty_broadcast_tag(
        &self,
    ) -> Result<[u8; RO_TAG_SIZE], MPECDSAError> {
...
impl TagRange {
    pub fn next(&mut self) -> Result<[u8; RO_TAG_SIZE], MPECDSAError> {
        if self.counter < self.length {
            ...
            Err(MPECDSAError::General(GeneralError::new(
                "Random Oracle tag range exhausted",
            )))
        }
    }
}
