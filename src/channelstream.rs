	...
	fn read_exact(&mut self, buf: &mut [u8]) -> Result<(), ::std::io::Error> {
		...
		if buf_len <= read_buf_len {
			...
			let mut received_vec = match self.inner.recv() {
				Ok(r) => r,
				Err(_) => {return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "Channel Hung Up."))},
			};
			let received_vals = &mut received_vec[..];
...
/// '''
pub fn spawn_n2_channelstreams(parties: usize) -> (Vec<Vec<Option<ChannelWriter>>>, Vec<Vec<Option<ChannelReader>>>) {
